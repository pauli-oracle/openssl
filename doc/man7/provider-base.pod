...
ossl_rand_get_entropy() and ossl_rand_cleanup_entropy() are used to get entropy
from the operating system.

get_entropy() is used to get entropy from the operating system.  The number
of bits of entropy is specified by B<entropy> and the generated entropy will
be at least B<min_len> and at most B<max_len> bytes in size.  The entropy
is returned in B<*pout>.  This function returns the length of the entropy
generated or zero on error.

cleanup_entropy() is used to clean up and free the entropy returned by
get_entropy().  The entropy pointer returned by get_entropy() is passed in
B<buf> and it's length in B<len>.

get_nonce() is used to generate a nonce from system parameters.  The generated
nonce will be at least B<min_len> and at most B<max_len> bytes in size.  A
provider generated B<salt> of length B<salt_len> bytes will be included in the
returned nonce.  The nonce is returned in B<*pout>.  This function returns the
length of the nonce or zero on error.

cleanup_nonce() is used to clean up and free the nonce returned by
